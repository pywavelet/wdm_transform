...
def test_wdm_time_roundtrip() -> None:
    series, nt, _ = _example_series()

    coeffs = WDM.from_time_series(series, nt=nt, a=1.0 / 3.0)
    recovered = coeffs.to_time_series()

    np.testing.assert_allclose(recovered.data, series.data, atol=1e-10, rtol=1e-10)


def test_wdm_frequency_reconstruction_matches_fft() -> None:
    series, nt, _ = _example_series()
    ...
    np.testing.assert_allclose(reconstructed.data, expected.data, atol=1e-10, rtol=1e-10)
