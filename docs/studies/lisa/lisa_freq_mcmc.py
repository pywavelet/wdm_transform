...
noise_psd_full = np.maximum(
    np.interp(freqs, freqs_saved, noise_psd_saved, left=noise_psd_saved[0], right=noise_psd_saved[-1]),
    1e-60,
)

print(f"Loaded injection from {INJECTION_PATH.name}")
print(f"T_obs = {t_obs / 86400:.1f} days  dt = {dt:.2f} s  N = {len(data_At)}  df = {df:.3e} Hz")
...
class BandData:
    ...
    band_kmin: int
    band_kmax: int
    prior_center: np.ndarray  # [f_c, fdot, phi_c] at midpoint time
    prior_scale: np.ndarray   # half-widths for each prior dimension
    f0_bounds: tuple[float, float]
    fdot_bounds: tuple[float, float]
...
def build_band(
    ...
    t_c = t_obs / 2.0
    f_c = params[0] + params[1] * t_c
    phi_c = (params[7] + 2 * np.pi * params[0] * t_c + np.pi * params[1] * t_c**2) % (2 * np.pi)
    ...
    return BandData(
        ...
        band_kmin=kmin,
        band_kmax=kmax,
        prior_center=np.array([f_c, params[1], phi_c]),
        prior_scale=np.array([
            0.25 * (prior_f0[1] - prior_f0[0]),
            0.25 * (prior_fdot[1] - prior_fdot[0]),
            np.pi / 2.0,
        ]),
        f0_bounds=tuple(prior_f0),
        fdot_bounds=tuple(prior_fdot),
...
BANDS = [
    build_band(
        ...
        prior_fdot=(SOURCE_PARAMS[0, 1] - 8e-18, SOURCE_PARAMS[0, 1] + 8e-18),
    ...
    build_band(
        ...
        prior_fdot=(SOURCE_PARAMS[1, 1] - 8e-18, SOURCE_PARAMS[1, 1] + 8e-18),
...
def sample_source(band: BandData, *, seed: int = 0) -> MCMC:
    ...
    def model():
        ...
        fdot = numpyro.deterministic("fdot", band.prior_center[1] + band.prior_scale[1] * delta_fdot)
        A = numpyro.deterministic("A", jnp.exp(logA))
        phi_c = jnp.mod(band.prior_center[2] + band.prior_scale[2] * phi_offset, 2 * jnp.pi)

        t_c = band.t_obs / 2.0
        f0 = numpyro.deterministic("f0", f_c - fdot * t_c)
        phi0 = numpyro.deterministic(
            "phi0",
            jnp.mod(phi_c - 2 * jnp.pi * f_c * t_c + jnp.pi * fdot * t_c**2, 2 * jnp.pi),
        )

        numpyro.factor(
            "f0_support",
            jnp.where((f0 >= band.f0_bounds[0]) & (f0 <= band.f0_bounds[1]), 0.0, -jnp.inf),
        )
        numpyro.factor(
            "fdot_support",
            jnp.where(
                (fdot >= band.fdot_bounds[0]) & (fdot <= band.fdot_bounds[1]), 0.0, -jnp.inf
            ),
        )
        ...
        params = (
            ...
            .at[1].set(fdot)
            .at[2].set(A)
            .at[7].set(jnp.mod(wrap_phase(phi0), 2 * jnp.pi))
        )
    ...
    kernel = NUTS(
        ...
        init_strategy=init_to_value(values={
            ...
            "delta_fdot": 0.0,
            "phi_offset": 0.0,
            "logA": float(np.log(band.fixed_params[2])),
        }),
...
for band, samples in zip(BANDS, all_samples):
    truth = np.array([
        band.fixed_params[0],
        band.fixed_params[1],
        band.fixed_params[2],
...
for ax, band, samples in zip(axes, BANDS, all_samples):
...
for band, samples, stem in zip(BANDS, all_samples, ["gb1_corner", "gb2_corner"]):
    truth = [band.fixed_params[0], band.fixed_params[1], band.fixed_params[2], band.fixed_params[7]]
    fig = corner.corner(
        samples,
