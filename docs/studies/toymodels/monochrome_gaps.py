...
outdir_monochrome_gaps = NOTEBOOK_DIR / "outdir_monochrome_gaps"
outdir_monochrome_gaps.mkdir(parents=True, exist_ok=True)
RUN_LOG_PATH = outdir_monochrome_gaps / "run_log.txt"
MARKDOWN_PATH = NOTEBOOK_DIR / "monochrome_gaps.md"
...
def save_figure(fig: plt.Figure, stem: str, *, dpi: int = 160) -> Path:
    ...
    path = outdir_monochrome_gaps / f"{stem}.png"
...
def choose_valid_wdm_shape(n_total: int, desired_nt: int) -> tuple[int, int]:
    ...
    nt = min(valid_nt, key=lambda candidate_nt: (abs(candidate_nt - desired_nt), -candidate_nt))
    nf = n_total // nt
    return nt, nf
...
print(
    f"Relative clean-signal WDM mismatch outside excluded bins: {outside_gap_mismatch:.3e}"
)
print(
    f"Relative clean-signal WDM mismatch inside excluded bins : {inside_gap_mismatch:.3e}"
)
...
axes[0].plot(
    times, gapped_data, color="tab:blue", linewidth=1.1, label="zero-filled gapped data"
)
for start, stop in gap_intervals:
...
axes[1].plot(
    freqs[positive], np.abs(np.asarray(full_fft.data))[positive], label="full data"
)
axes[1].plot(
    freqs[positive],
...
noise_realizations = np.stack(
    [
        np.asarray(
            TimeSeries(
                random_signal_from_psd(stationary_noise_psd, n_total, dt, RNG)
                * sample_mask,
                dt=dt,
            )
            .to_wdm(nt=nt)
...
axes[0].plot(
    ...
    color="tab:green",
    linewidth=1.3,
    label="exact masked time",
)
axes[0].plot(
    freq_rfft, fft_fit_mag, color="tab:purple", linewidth=1.3, label="gap-ignorant FFT"
)
axes[0].plot(
    freq_rfft, wdm_fit_mag, color="tab:orange", linewidth=1.3, label="WDM kept-bins"
)
...
axes[1].plot(
    freq_rfft,
    np.abs(gapped_fft_mag - truth_fft),
    color="tab:blue",
    alpha=0.7,
    label="gapped data error",
)
axes[1].plot(
    freq_rfft,
    np.abs(exact_fft_mag - truth_fft),
    color="tab:green",
    label="exact masked time error",
)
axes[1].plot(
    freq_rfft,
    np.abs(fft_fit_mag - truth_fft),
    color="tab:purple",
    label="gap-ignorant FFT error",
)
axes[1].plot(
    freq_rfft,
    np.abs(wdm_fit_mag - truth_fft),
    color="tab:orange",
    label="WDM kept-bins error",
)
axes[1].set_xlim(0.0, 0.5 / dt)
