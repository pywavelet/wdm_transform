...
outdir_deglitching = NOTEBOOK_DIR / "outdir_deglitching"
outdir_deglitching.mkdir(parents=True, exist_ok=True)
RUN_LOG_PATH = outdir_deglitching / "run_log.txt"
MARKDOWN_PATH = NOTEBOOK_DIR / "deglitching.md"
...
def save_figure(fig: plt.Figure, stem: str, *, dpi: int = 160) -> Path:
    ...
    path = outdir_deglitching / f"{stem}.png"
...
def inject_glitches(
    ...
    for center, amp, width, carrier_frequency, phase in zip(
        ...
        carrier = np.cos(
            2.0 * np.pi * carrier_frequency * (times - times[center]) + phase
        )
...
axes[0].plot(
    times, observed, color="tab:red", alpha=0.7, label="observed with glitches"
)
for center in glitch_centers:
    axes[0].axvline(
        center * dt, color="black", linestyle="--", linewidth=1.0, alpha=0.5
    )
...
axes[2].plot(
    bin_times,
    mask_soft * np.max(time_score),
    color="tab:orange",
    label="soft mask (scaled)",
)
...
axes[1].plot(
    times, cleaned, color="tab:orange", linewidth=1.1, label="one-pass cleaned"
)
axes[1].plot(
    times, iter_cleaned, color="tab:green", linewidth=1.3, label="iterative cleaned"
)
for center in glitch_centers:
    axes[1].axvline(
        center * dt, color="black", linestyle="--", linewidth=1.0, alpha=0.4
    )
...
axes[0].plot(
    times, reference, color="tab:blue", alpha=0.7, label="reference (signal + noise)"
)
...
axes[0].plot(
    times, iter_cleaned, color="tab:green", alpha=0.9, label="iterative cleaned"
)
...
for ax, center in zip(axes[1:], glitch_centers):
    ...
    ax.plot(
        times[mask], reference[mask], color="tab:blue", alpha=0.7, label="reference"
    )
...
for _ in range(2):
    inference_coeffs = np.asarray(
        TimeSeries(inference_current, dt=dt).to_wdm(nt=nt).coeffs
    )
    inference_detection = detect_glitches(inference_coeffs, dt=dt)
    inference_coeff_mask = (np.asarray(inference_detection["whitened"]) > 2.5).astype(
        float
    )
